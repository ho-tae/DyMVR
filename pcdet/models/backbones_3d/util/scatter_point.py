    ...
    def forward(self, points, coors):
        ...
        if coors.size(-1) == 3:
            ...
            for i in range(batch_size):
                inds = torch.where(coors[:, 0] == i) # 배치 0에 해당되는 부분의 index를 찾아줌
                voxel, voxel_coor = self.forward_single(
                    points[inds], coors[inds][:, 1:]) # 배치에 해당되는 값을 배치 빼고 dynamic scatter에 넣어줌
                coor_pad = nn.functional.pad(
                    voxel_coor, (1, 0), mode='constant', value=i) # 그리고 voxel을 처리한 후 배치를 다시 할당
