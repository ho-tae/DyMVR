...
class VoxelResBackBone_low(nn.Module):
    def __init__(self, input_channels, grid_size, **kwargs):
        super().__init__()
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv2 = spconv.SparseSequential(
            # [2, 550, 550] <- [1, 275, 275]
            block(32, 64, 3, norm_fn=norm_fn, stride=[2, 2, 2], padding=[1, 1, 1], indice_key='spconv2', conv_type='spconv'),
        ...
        self.conv3 = spconv.SparseSequential(
            # [1, 275, 275] <- [1, 138, 138]
            block(64, 128, 3, norm_fn=norm_fn, stride=[2, 2, 2], padding=[1, 1, 1], indice_key='spconv3', conv_type='spconv'),
    ...
    def forward(self, batch_dict):
        ...
        Returns:
            batch_dict:
                encoded_spconv_tensor: sparse tensor
        """
        voxel_features, voxel_coords = batch_dict[1], batch_dict[2]
        batch_size = batch_dict[0]
        input_sp_tensor = spconv.SparseConvTensor(
...
class VoxelResBackBone_mid(nn.Module):
    def __init__(self, input_channels, grid_size, **kwargs):
        super().__init__()
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv2 = spconv.SparseSequential(
            # [1, 275, 275] <- [2, 550, 550]
            block(64, 128, 3, norm_fn=norm_fn, stride=[2, 2, 2], padding=[1, 1, 1], indice_key='spconv2', conv_type='spconv'),
...
class VoxelBackBone8x_low(nn.Module):
    def __init__(self, input_channels, grid_size, **kwargs):
        super().__init__()
        #self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
...
class VoxelBackBone8x_dim(nn.Module):
    def __init__(self, input_channels, grid_size, **kwargs):
        super().__init__()
        #self.model_cfg = model_cfg
        norm_fn = partial(nn.BatchNorm1d, eps=1e-3, momentum=0.01)
        ...
        self.conv_input = spconv.SparseSequential(
            spconv.SubMConv3d(input_channels, 224, 3, padding=1, bias=False, indice_key='subm1'),
            norm_fn(32),
            nn.ReLU(),
        )
        block = post_act_block
