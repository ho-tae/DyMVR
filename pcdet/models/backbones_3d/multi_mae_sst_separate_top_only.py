...
from pcdet.models.sst.sst_basic_block import BasicShiftBlock
from pcdet.ops.sst.sst_ops import flat2window, window2flat, scatter_v2
...
class MultiMAESSTSPChoose(nn.Module):
    ...
    def __init__(
        ...
        for i in range(decoder_num_blocks):
            decoder_distance_block_list.append(
                BasicShiftBlock(d_model[i], nhead[i], dim_feedforward[i],
                    dropout, activation, batch_first=False, block_id=i)
            )    
        ...
        self.decoder_distance_blocks = nn.ModuleList(decoder_distance_block_list)
        ...
        self.num_chamfer_points = 10
        self.pred_dims = 3
        self._init_layers()

        # build a 2-layer predictor
        #self.predictor = nn.Sequential(nn.Linear(self.d_model[0], self.d_model[0], bias=False),
        #                               nn.BatchNorm1d(self.d_model[0]),
        #                               nn.ReLU(inplace=True), # hidden layer
        #                               nn.Linear(self.d_model[0], self.d_model[0])) # output layer
        
    def _init_layers(self):
        """Initialize neural network layers of the head."""
        self.conv_chamfer = nn.Conv1d(self.d_model[0], self.num_chamfer_points * self.pred_dims, 1) \
    ...
    def forward(self,
        ...
        #top_attention_output = self.forward_encoder(visible_tokens_tuple)
        top_visible_tokens_tuple = self.get_voxel_info(voxel_feat, coors, batch_size)
        #mid_low_visible_tokens_tuple = self.get_voxel_info(voxel_mid_low_features, coors, batch_size)
        attention_output = self.forward_encoder(top_visible_tokens_tuple, voxel_mid_low_features)
        #top_output = self.forward_encoder(top_tokens_tuple)        
        #p1 = self.predictor(top_attention_output)
        #p2 = self.predictor(top_output)
        #top_output = top_output + top_attention_output
        
        x = self.forward_decoder(attention_output, coors, coors_mask, gt_dict, batch_size)
        return x #,[p1, p2, top_attention_output.detach(), top_output.detach()]
    ...
    def forward_encoder(self, input_tuple, voxel_mid_low_features=None):
        '''
        '''
        ...
        low_mid_feat = voxel_mid_low_features
        ...
        for i, block in enumerate(self.encoder_blocks):
            top_output = block(top_output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                padding_mask_list, self.drop_info)    
            
        mid_low_output = low_mid_feat
        for i, block in enumerate(self.encoder_blocks):
            mid_low_output = block(mid_low_output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                padding_mask_list, self.drop_info)   
        
        output = torch.cat([top_output, mid_low_output])
        
        for i, block in enumerate(self.encoder_blocks):
            output = block(output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                padding_mask_list, self.drop_info, cross_atten=True)
            if i != len(self.encoder_blocks)-1:
                output = torch.cat([top_output, output])
        return output


    def forward_decoder(self, visible_voxel_feat, coors, coors_mask, gt_dict, batch_size):
        masked_start_id=coors.shape[0] # [1376, 4] -> id: 1376
        mask_tokens = self.mask_token.repeat(coors_mask.shape[0], 1) # [3213, 128]
        # print(voxel_feat.shape,mask_tokens.shape)
        voxel_feat_ = torch.cat([visible_voxel_feat, mask_tokens],dim=0) # [4589, 128]
        coors_ = torch.cat([coors, coors_mask],dim=0) # [4589, 4]
        ...
        for i, block in enumerate(self.decoder_centroid_blocks):
            centroid_output = block(centroid_output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                           padding_mask_list, self.drop_info, encoder)
        ...
        for i, block in enumerate(self.decoder_distance_blocks):
            distance_output = block(distance_output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                           padding_mask_list, self.drop_info, encoder)
            
        centroid_masked_output = centroid_output[masked_start_id:]
        density_masked_output = density_output[masked_start_id:]
        distance_masked_output = distance_output[masked_start_id:]
        
        pred_dict = {}
        gt_points_per_voxel = gt_dict["points_per_voxel"]
        gt_points_padding = gt_dict["points_per_voxel_padding"]
        
        pred_points_masked = self._apply_1dconv(self.conv_chamfer, distance_masked_output).view(
            len(distance_masked_output), self.num_chamfer_points, self.pred_dims)
        pred_points_masked = torch.tanh(pred_points_masked)  # map to [-1, 1]
        
        gt_points_masked = gt_points_per_voxel[gt_dict["ids_mask"]]
        gt_point_padding_masked = gt_points_padding[gt_dict["ids_mask"]]
        ...
        if self.cls_sub_voxel:
            ...
            return reg_pred_low,reg_pred_med,reg_pred_top,reg_pred_nor_low,reg_pred_nor_med,reg_pred_nor_top,cls_pred_low,cls_pred_med,pred_dict
        else:
    ...
    def get_pos_embed(self, ind_dict, coors_in_win, voxel_drop_level, dtype, voxel_window_level):
        ...
        # [N,]
        win_x, win_y, _ = self.window_shape
    ...
    def window_partition(self, coors, voxel_info):
        ...
        win_shape_x, win_shape_y, _ = self.window_shape
