...
from pcdet.ops.sst.sst_ops import get_inner_win_inds
...
class SSTInputLayerV2Masked(SSTInputLayerV2):
    ...
    def forward(
        ...
        """
        Args:
            voxel_feats: shape=[N, C], N is the voxel num in the batch, ex:[25491, 128]
            voxel_coors: shape=[N, 4], [b, z, y, x], voxel coordinate for each voxel, ex:[25491, 4]
            ...
            point_coors: shape=[Np, 4], [b, z, y, x], voxel coordinate for each point ex:[121513, 4]
        Returns:
        ...
        """
        batch_size = batch_size #int(voxel_coors[:, 0].max().item()) + 1
    ...
    def mask_voxels(self, device, voxel_coors, voxel_feats, fake_voxel_coors):
        ...
        mask = (
            torch.rand(len(voxel_feats), device=device) < self.masking_ratio
        )  # 랜덤으로 비율보다 작게 mask 해주고
        ...
        if self.use_fake_voxels:
            ...
            fake_voxel_feats = torch.zeros(
                (len(fake_voxel_coors), voxel_feats.shape[1]),
                device=device,
            ...
            mask = torch.cat(
                [
                    mask,
                    torch.zeros(len(fake_voxel_coors), device=device, dtype=torch.bool),
                ]
            )
        ...
        voxel_info_encoder = super().forward(
            unmasked_voxels, unmasked_voxel_coors, batch_size=None
        )
        assert (
            len(voxel_info_encoder["voxel_feats"]) == n_unmasked_voxels
        ), "Dropping is not allowed for reconstruction"
        ...
        if self.debug:
            ...
            assert torch.allclose(
                decoder_feats[dec2dec_input_idx], voxel_feats
            ...
            assert torch.allclose(
                decoder_feats[dec2masked_idx], voxel_feats[masked_idx]
            ...
            assert torch.allclose(
                decoder_feats[dec2enc_idx], encoder_feats
            ), "The mapping from decoder to encoder output is invalid"
            assert torch.allclose(
                decoder_coors[dec2enc_idx], encoder_coors.long()
            ), "The mapping from decoder to encoder output is invalid"
    ...
    def get_ground_truth(
        ...
        if self.use_num_points:
            n_points_per_voxel_with_zeros = torch.bincount(point_indices) # torch.bincount 함수를 사용하여 포인트 인덱스의 빈도를 계산
            point_indices_unique = n_points_per_voxel_with_zeros.nonzero().ravel() #  0이 아닌 값을 갖는 원소의 인덱스를 반환, revel을 사용하여 배열을 1차원 배열로 변환
            n_points_per_voxel = n_points_per_voxel_with_zeros[voxel_indices] # 복셀에 속하는 포인트 수를 얻음
            gt_dict["num_points_per_voxel"] = n_points_per_voxel # 그 후 gt_dict으로 저장
            assert (
                n_points_per_voxel > 0
            ).all(), "Exists voxel without connected points"
            assert len(point_indices_unique) == len(
                voxel_indices
            ), "There is a mismatch between point indices and voxel indices"
            assert (
                point_indices_unique == voxel_indices.sort()[0]
            ).all(), "There is a mismatch between point indices and voxel indices"
        ...
        if self.use_chamfer:
            ...
            gt_points_padding[
                dropped_point_indices, inner_voxel_inds
            ] = 0  # not_padded -> 0, padded -> 1
        ...
        if self.use_chamfer and self.use_num_points:
            ...
            assert (
                _n_points_per_voxel[test_mask] == n_points_per_voxel[test_mask]
            ).all(), (
                "Mismatch between counted points per voxel and found points per voxel"
            )
            assert (
                _n_points_per_voxel[~test_mask] == self.drop_points_th
            ).all(), "Error when dropping points for voxels with to many points"
        ...
        if self.use_fake_voxels:
            ...
            n_fake_voxels_per_batch = (
                voxels_per_batch * self.fake_voxel_ratio
            ...
            fake_voxel_idxs = [
                torch.where(~occupied[b])[0] + (b * max_num_voxels_per_batch)
                for b in range(batch_size)
            ]
            fake_voxel_idxs = torch.cat(
                [
                    idx[torch.randperm(len(idx), device=device)][
                        :n_fake
            ...
            fake_voxel_coors = torch.zeros(
                (n_fake_voxels, voxel_coors.shape[1]),
                device=device,
                dtype=voxel_coors.dtype,
            )
            fake_voxel_coors[:, 0] = fake_voxel_idxs // (vz * vy * vx)  # batch index
            fake_voxel_coors[:, 1] = (fake_voxel_idxs % (vz * vy * vx)) // (
                vy * vx
            )  # z index
            ...
            mask = torch.zeros(
                (len(voxel_coors) + len(fake_voxel_coors)),
                device=device,
                dtype=torch.float,
            )
            mask[len(voxel_coors) :] = 1.0
