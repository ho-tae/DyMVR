...
class SSTv2Decoder(SSTv2):
    ...
    def forward(self, voxel_info):
        ...
        voxel_feat = voxel_info_decoder['voxel_feats'].to('cuda:0')
        voxel_feat[dec2enc_idx] = encoder_out.to('cuda:0')
        ...
        masked_tokens = self.mask_token.repeat(n_masked, 1).to(voxel_feat.device)
        ...
        if self.use_fake_voxels:
            ...
            masked_tokens = self.mask_token.repeat(n_fake, 1).to(voxel_feat.device)
            voxel_feat[dec2fake_idx] = masked_tokens
        voxel_info_decoder['batch_size'] = voxel_info['batch_size']
        voxel_info_decoder['voxel_feats'] = voxel_feat

        if self.debug:
            test_mapping = -torch.ones(len(voxel_feat), device=voxel_feat.device) ## 22919
            test_mapping[dec2enc_idx] = 0 # 6277
            test_mapping[dec2masked_idx] = 1 # 14559
            if self.use_fake_voxels:
